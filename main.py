...
def resizeAndSaveNifti(path, size, inter):
    size = 128
    new_size_y = 128
...
def resizeNiftiFolder(folder, size=128, inter=cv2.INTER_CUBIC):
    if not os.path.exists('resized/'+folder): os.makedirs('resized/'+folder)
    for filename in os.listdir(folder):
        img = resizeAndSaveNifti(folder+filename, size, inter)
        print(folder+filename);
        img.to_filename('resized/'+folder+filename)

#resizeNiftiFolder('Task03_Liver/imagesTr/')

resizeNiftiFolder('Task03_Liver/labelsTr/', inter=cv2.INTER_NEAREST)
